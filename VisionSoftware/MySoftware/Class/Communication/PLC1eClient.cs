...
namespace MySoftware
{
    ...
    public class PLC1eClient
    {
        ...
        public PLC1eClient(string plcAddress, int plcPort, string dataIn, int dataInLength, string dataOut, int dataOutLenght) : this()
        {
            UpdateDataLinkAddress(dataIn, dataInLength, dataOut, dataOutLenght);
            PLCClientInit(plcAddress, plcPort);
        }

        public PLC1eClient(string plcAddress, int plcPort)
        {
            PLCClientInit(plcAddress, plcPort);
        }
        ...
        public void PLCClientInit(string ipAddress, int port)
        {
            ...
            try
            {
                ...
                PLCSocket.Connect(serverAddress, port);
        ...
        public int[] ReceiveDataFromPLCTest(int wordAddress, int numberData)
        {
            NetworkStream stream = PLCSocket.GetStream();
            string RxResponse;
            string Temp;
            string SubHeader;
            string DregStr;
            double[] Dreg = new double[5];
            int j = 0;
            string TxCommand = "01ff000a4420000000000500";
            int[] dataOutArr = new int[numberData];
            char[] arrChar = TxCommand.ToCharArray();
            byte [] Buffer = Encoding.Default.GetBytes(arrChar);
            ...
            SubHeader = RxResponse.Substring(3, 2);
            if (SubHeader == "00") //Normal response
            {
                Temp = "";
                for (int i = 0; i <= 4; i++)
                {
                    DregStr = RxResponse.Substring(i * 4 + 5, 4);
                    bool rs = double.TryParse(DregStr,out double data );
                    if (rs)
                    {
                        Dreg[i] = data;
                    }
            ...
            else if (SubHeader == "5B") // In an abnormal response, an abnormal code is added
            {
